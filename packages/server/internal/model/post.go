...
type Post struct {
	...
	ID int64 `gorm:"primaryKey;autoIncrement:false;index:idx_posts_cursor,sort:desc,priority:2;index:idx_posts_author_feed,sort:desc,priority:3;index:idx_posts_partition_feed,sort:desc,priority:3" json:"id,string"`
	...
	CreatedAt time.Time `gorm:"index:idx_posts_cursor,sort:desc,priority:1,where:deleted_at IS NULL;index:idx_posts_author_feed,sort:desc,priority:2;index:idx_posts_partition_feed,sort:desc,priority:2" json:"created_at"`
