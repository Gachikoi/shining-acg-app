<script lang="ts">
	...
	import { COMMENT_MENTION_ACTION_LABELS, type CommentMentionNotificationItem } from './types';

	let {
		item,
		onOpenNotificationTarget
	}: {
		item: CommentMentionNotificationItem;
		onOpenNotificationTarget?: (payload: { type: string; id: string }) => void;
	} = $props();

	const actionLabel = $derived(COMMENT_MENTION_ACTION_LABELS[item.actionType]);
...
>
	...
	<div class="min-w-0 flex-1">
		...
		{#if item.body}
			<p class="mt-1 text-sm leading-relaxed text-zinc-800 dark:text-zinc-200">{item.body}</p>
		{/if}

		{#if item.replyQuote}
			<p
				class="mt-2 truncate border-l-2 border-zinc-200 pl-2 text-xs text-zinc-400 dark:border-zinc-700"
			>
				{item.replyQuote}
			</p>
		{/if}
	...
	<TargetPreview
		...
		quote={item.replyQuote ? undefined : item.body}
	/>
</div>
