...
在布局中挂载一次 `StackContainer`，业务里通过默认导出的 `stackController`
操作栈。
...
**业务侧：**一般只写 `stackController.pop()`
即可——默认就是「带出场动画」的出栈，与产品预期一致。**不要**在业务里传
`false`，除非明确要在无过渡下立刻摘掉栈顶。

`false` 主要留给 **Stack 内部**（如 `StackItem` 手势提交、动画收尾后真正从数组
`pop`）使用；业务代码几乎用不到。
...
- `true`
  时函数在触发动画后即返回，**不**返回被弹出项（实现上此时尚未从数组移除）。
